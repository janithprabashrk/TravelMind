...
class DatabaseManager:
    ...
    def save_user_feedback(self, user_id: str, hotel_id: int, recommendation_id: int, rating: float, feedback_text: str = "") -> int:
        """Save user feedback"""
        if self.db_type == 'mysql':
            from sqlalchemy import text
            with self.engine.connect() as conn:
                result = conn.execute(text("""
                    ...
                    VALUES (:user_id, :hotel_id, :recommendation_id, :rating, :feedback_text)
                """), {
                    'user_id': user_id,
                    'hotel_id': hotel_id,
    ...
    def save_recommendation_results(self, user_id: str, algorithm: str, recommendations: List[Dict[str, Any]]) -> List[int]:
            ...
            with self.engine.connect() as conn:
                for rec in recommendations:
                    result = conn.execute(text("""
                        ...
                        VALUES (:user_id, :hotel_id, :score, :recommendation_type)
                    """), {
                        'user_id': user_id,
                        'hotel_id': rec.get('id'),
                    ...
                    recommendation_ids.append(result.lastrowid or 0)
                conn.commit()
        else:
            cursor = self.connection.cursor()
            for rec in recommendations:
                cursor.execute("""
                    INSERT INTO recommendations (user_id, hotel_id, score, recommendation_type)
                    VALUES (?, ?, ?, ?)
                """, (user_id, rec.get('id'), rec.get('score', 0.0), algorithm))
                recommendation_ids.append(cursor.lastrowid or 0)
            self.connection.commit()
